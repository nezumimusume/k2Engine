...
float4 PSMain(PSInput In) : SV_Target0
{
	...
	//後で定数バッファにするんやで？
	int maxRayNum = 20;
	float3 raystep = 200.0f/maxRayNum * toPixelDirReflect;
	...
	for( int step = 1; step < maxRayNum; step++){ //自分自身とぶつかるので、stepは1から進める。
		...
		if (depth < pixelZ && posInProj.z - pixelZ > 0 && posInProj.z - pixelZ < maxThickness) {
			float sign = -1.0;
			for (int m = 1; m <= 4; ++m) {
			...
			return float4(reflectColor.xyz, metallicSmooth.a * 0.8f);
		}
	}
	clip(-1.0f);
